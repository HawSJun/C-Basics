...
### 객체지향(OOP, Object-Oriented Programming) VS 절차지향(POP, Procedural-Oriented Programming)
...
### 전처리기 #define : [전처리기](./CBasics/conditional.cpp)
...
### 변수(Variable) : [변수](./CBasics/variable.cpp) / [변수](./Day1/value.c)

- 변수 :  값을 저장할 수 있는 메모리 공간, 프로그램의 데이터를 저장하는 기본 단위!

    - **지역 변수** : 함수 내부에 선언된 변수를 의미. 해당 블록 내부에서만 유효하며, 블록이 끝나면 메모리에서 해제
    ...
    - **전역 변수** : 함수 밖에서 선언된 변수. 프로그램이 시작될 때 생성되며, 프로그램이 종료될 때까지 메모리에 존재

    - 정적 변수 : static 키워드를 사용
...
- Rvalue : 우변에만 올 수 있는 값, 보통 임시적인 값을 의미. 즉, 특정한 메모리에 저장되지 않거나, 한 번 생성된 후 더 이상 변경되지 않는 값
    ...
    - `리터럴(Literal)` : 값 자체를 의미하는 상수
        ...
        const char* str = "Hello"; // "Hello"는 문자열 리터럴인데, 문자열 리터럴은 예외적으로 Lvalue입니다.
        ```
